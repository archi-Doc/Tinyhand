        ...
        internal void GenerateInitSetters(ScopingStringBuilder ssb, GeneratorInformation info)
        {
            ...
            using (var scopeMethod = ssb.ScopeBrace($"private static bool {initializeMethod}()"))
            {
                ...
                foreach (var x in array)
                {
                    ...
                    ssb.AppendLine($"{x.InitDelegateIdentifier} = FastExpressionCompiler.ExpressionCompiler.CompileFast(Expression.Lambda<Action<{this.LocalName}, {x.TypeObject!.FullName}>>(Expression.Call(expType, mi!, exp), expType, exp));");
                }
