...
internal static class TinyhandParser
{
    ...
    public static Element ParseFile(string fileName, TinyhandParserOptions? options = null)
    {
        ...
        fs.Read(buffer, 0, buffer.Length); // fs.Read(buffer.AsSpan());
    ...
    public static async Task<Element> ParseFileAsync(string fileName, TinyhandParserOptions? options = null)
    {
        ...
        var length = fs.Length;
        var buffer = new byte[length];
        await fs.ReadAsync(buffer, 0, buffer.Length); // await fs.ReadAsync(buffer.AsMemory());
    ...
    public static Element Parse(Stream stream, TinyhandParserOptions? options = null)
    {
        ...
        {// Other
            ...
            var readBytes = stream.Read(buffer, 0, buffer.Length); // stream.Read(span);
    ...
    public static Element Parse(string text, TinyhandParserOptions? options = null) => Parse(Encoding.UTF8.GetBytes(text), options);
}
