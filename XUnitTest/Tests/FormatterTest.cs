...
namespace XUnitTest
{
    public class FormatterTest
    {
        ...
        public void DateTimeOffsetTest()
        {
            ...
            var binary = TinyhandSerializer.Serialize(now);
            TinyhandSerializer.Deserialize<DateTimeOffset>(binary).Is(now);
        }
        ...
        public void StringTest_Part2()
        {
            ...
            var sequenceAWriter = new TinyhandWriter();
            sequenceAWriter.Write(a);
            var sequenceA = sequenceAWriter.FlushAndGetArray();
            sequenceA.Length.Is(Encoding.UTF8.GetByteCount(a) + 1);

            var sequenceBWriter = new TinyhandWriter();
            sequenceBWriter.Write(b);
            var sequenceB = sequenceBWriter.FlushAndGetArray();
            sequenceB.Length.Is(Encoding.UTF8.GetByteCount(b) + 2);

            var sequenceCWriter = new TinyhandWriter();
            sequenceCWriter.Write(c);
            var sequenceC = sequenceCWriter.FlushAndGetArray();
            sequenceC.Length.Is(Encoding.UTF8.GetByteCount(c) + 3);

            var sequenceDWriter = new TinyhandWriter();
            sequenceDWriter.Write(d);
            var sequenceD = sequenceDWriter.FlushAndGetArray();
            sequenceD.Length.Is(Encoding.UTF8.GetByteCount(d) + 5);

            var readerA = new TinyhandReader(sequenceA);
            var readerB = new TinyhandReader(sequenceB);
            var readerC = new TinyhandReader(sequenceC);
            var readerD = new TinyhandReader(sequenceD);
        ...
        public void DecimalLang()
        {
            ...
            try
            {
                ...
                var b = TinyhandSerializer.Serialize(12345.6789M);
                var d = TinyhandSerializer.Deserialize<decimal>(b);
